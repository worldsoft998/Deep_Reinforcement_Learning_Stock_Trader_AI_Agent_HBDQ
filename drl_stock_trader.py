...
class DQN_Agents(object):
    ...
    def __init__(self, state_size_, action_size_):
        ...
        self.memory = ReplayingBuffers(state_size_, action_size_, size=500)
        self.gamma = 0.95
        self.epsilon = 1.0
...
if __name__ == '__main__':
    ...
    """
    # Config
    models_folder = 'drl_trader_models'  # stores models
    rewards_folder = 'drl_trader_rewards'  # stores rewards
    num_episodes = 20
    batch_size = 32
    ...
    makingdir(models_folder)
    makingdir(rewards_folder)
    ...
    env = StockTradeEnviron(train_data, initial_investment)
    state_size = env.state_dim
    action_size = len(env.action_space)
    agent = DQN_Agents(state_size, action_size)
    scaler = gettingscaler(env)
    ...
    if args.mode == 'test':  # test mode
        ...
        # Remake environment
        env = StockTradeEnviron(test_data, initial_investment)
    ...
    for e in range(num_episodes):
        t0 = datetime.now()
        val = play_one_instance(agent, env, args.mode)
        dt = datetime.now() - t0
